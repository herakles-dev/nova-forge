...
|-------|-------|-------|------|-------|
| Nova Lite (32K) | S | 100% | 170s | 24 |
| Nova Pro (300K) | S | 99% | 130s | 13 |
| Nova Premier (1M) | S | 100% | 443s | 10 |

### Remaining Work
- **Fine-tuning**: Additional benchmark scenarios, edge case hardening
- **Benchmarks**: Expand beyond Expense Tracker (new task sets, different stacks)
- **Website**: forge.herakles.dev presentation quality — demo recording, visual polish, content
- **Submission**: Devpost, demo video, gallery apps (deadline March 16)

## Commands

```bash
# Tests
pytest tests/ -x -q                                      # Quick (1051 tests)
...
- **Testing**: pytest (1051 tests, 48 test files)
...
## Module Map (38 files, 24,534 LOC)

| Module | LOC | Purpose |
|--------|-----|---------|
| forge_cli.py | 3604 | Interactive shell, deep planning interview, all /commands |
| forge_agent.py | 1638 | Tool-use loop, 12 tools, AgentEvent, auto-verify, read-before-write |
...
| prompt_builder.py | 917 | 3-tier prompt system + autonomy-aware + previewability |
| model_router.py | 902 | 3 provider adapters (Bedrock 300s timeout, OpenAI, Anthropic) |
...
| forge_display.py | 604 | Rich live UI, brand-themed spinners and progress |
| forge_verify.py | 566 | BuildVerifier — L1 static, L2 server, L3 browser checks |
| forge_deployer.py | 468 | Docker + nginx + SSL deployment |
| forge_web.py | 325 | Web dashboard + docs chat API |
| config.py | 320 | Model configs, context windows, scaled max_tokens |
...
| forge_prompt.py | 281 | Selection menus, Escape-to-cancel, brand colors |
...
| forge_competition.py | 210 | Competition readiness validator (8 checks) |

### Benchmark & Demo Scripts

| Module | LOC | Purpose |
|--------|-----|---------|
| benchmark_nova_models.py | 1341 | Model benchmark — auto-save, regressions, pre-seeded context |
...
├── tests/unit/                 # 1051 tests (48 test files)
...
- **Test after changes**: `pytest tests/ -x -q` (1051 tests, all must pass)
